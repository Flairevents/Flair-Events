...
class PublicController < ApplicationController
  ...
  def contact
    ...
    if request.post?
      ...
      if @errors.empty?
        ...
        flash[:notice] = "Thank you! Your message has been delivered. Flair office staff will get back to you as quickly as possible. If urgent, you can always call us at #{strip_tags(TextBlock['company-tel'])}."
        redirect_to "/"
        return
      else
  ...
  def events
    ...
    if request.xhr?
      render json: {contents: render_to_string(partial: 'shared/event_list', locals: {events: @events, featured_events: @featured_events})}
    else
    ...
    if token = params[:token]
      ...
      if email.blank? || password.blank?
        ...
        if result == :success
          ...
          flash.now[:error] = "You need to confirm your e-mail address by clicking on the login link we sent you first. We're sending it to you again right now. Please check your e-mail inbox."
          user = Prospect.find_by_email(params[:login_email])
          user.account.generate_one_time_token!
          send_mail(PublicMailer.remind_to_confirm_email(user, user.account))
  ...
  def register
    ...
    if request.post?
      fields = [:first_name, :last_name, :email, :email_confirm, :password, :password_confirm, :mobile, :post_code, :city]
      fields.each { |k| params[k].strip! if params[k] }
      fields.each { |k| params.delete(k) if params[k].blank? }
      params[:email] = (params[:email] || '').gsub(/\s+/, '').downcase
      params[:email_confirm] = (params[:email_confirm] || '').gsub(/\s+/, '').downcase
      ...
      if (user = Prospect.find_by_email(params[:email])) && user.account
        ...
        back_to_register
        @password = @password_confirm = nil
      elsif !params[:agree_to_mail]
        flash.now[:error] = "To apply for work with Flair Event Staffing, you must agree to receive work-related e-mails from us. You can deactivate your account at any time if you do not want to receive these e-mails any more."
        back_to_register
        ...
        if params[:mobile].blank? || params[:mobile].length < 7
          ...
          if date_hash_blank?(params[:date_of_birth]) ||
             !(date = hash_to_date(params[:date_of_birth])) ||
             !((91.years.ago..5844.days.ago).cover?(date))
            flash.now[:error] = "Please enter a valid date of birth. You must be at least 16 years old to register."
            back_to_register
          else
            if user = Prospect.where(email: params[:email]).first
              ...
              user.date_of_birth = hash_to_date(params[:date_of_birth])
              user.mobile_no  = params[:mobile]
            else
              user = Prospect.new(first_name: params[:first_name], last_name: params[:last_name], email: params[:email], post_code: params[:post_code], date_of_birth: hash_to_date(params[:date_of_birth]), mobile_no: params[:mobile], applicant_status: 'UNCONFIRMED', city: params[:city])
            end
  ...
  def back_to_register
    ...
    @agree_to_mail, @agree_to_policy = params[:agree_to_mail], params[:agree_to_policy]
    @mobile = params[:mobile]
    if !date_hash_blank?(params[:date_of_birth])
      @date_of_birth = hash_to_date(params[:date_of_birth])
    end
  ...
  def bypass_registration(user)
    #Rails.env.staging? || (/@gmail/.match(user.email) != nil)
    Rails.env.staging?
  end
