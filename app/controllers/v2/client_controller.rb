...
class V2::ClientController < ApplicationController
  layout 'v2/layouts/client'

  only_accessible_to_client_contacts except: [:activate, :login, :index]
  ...
  def login
    ...
    if request.variant.present?
      return render 'v2/mobile/client/login'
    end
  end
  ...
  def index
    if request.variant.present?
      return render 'v2/mobile/client/index'
    end
  end
end